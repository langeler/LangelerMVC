...
- Run `php console framework:architecture` or `composer architecture:check` when repository contract paths, App layer boundaries, class placement, public/bootstrap entrypoints, config/data/release parity, tests/CI/scripts, manager placement, support alias corridors, module directory shape, `.vide` presentation conventions, class-file strictness, or docs source-of-truth rules move; this feeds the `architecture_alignment` release-check section.
