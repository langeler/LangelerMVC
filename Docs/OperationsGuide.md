...
- The `architecture_alignment` release-check section is backed by `php console framework:architecture`, which verifies repository contract paths, App layer boundaries, class placement, public/bootstrap thinness, config/data/release parity, tests/CI/scripts, strict class files, canonical manager placement, support alias corridors, documented module shape, native `.vide` presentation rules, and documentation alignment.
