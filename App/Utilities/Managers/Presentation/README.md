...
- `TemplateEngine`: compiles `.vide` and `.lmv` templates into cached PHP render targets.
- `ThemeManager`: resolves configured themes, mode defaults, public assets, and shared layout globals.

First-party templates should prefer `.vide` directives such as `@style`, `@script`, `@image`, `@preload`, `@assetBundle`, `@csrf`, `@method`, `@class`, `@attr`, and `@json` instead of raw PHP tags. `App/Support/Theming/ThemeManager.php` remains as a backward-compatible alias for older projects.
