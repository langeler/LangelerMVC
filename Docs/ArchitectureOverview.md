...
- full-repo architecture alignment through `ArchitectureAlignmentManager`, `php console framework:architecture`, and the `architecture_alignment` release-check gate, covering repository contract paths, App boundaries, class placement, public/bootstrap thinness, config/data/release parity, tests/CI/scripts, strict class files, manager placement, support alias corridors, module shape, native presentation, and docs
...
- **Architecture Alignment**: update `ArchitectureAlignmentManager` and `Docs/ArchitectureAlignment.md` whenever class-placement, strict class-file, canonical manager, support alias, module-shape, presentation-native, or documentation source-of-truth rules intentionally change.
